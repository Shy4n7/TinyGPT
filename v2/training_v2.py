...
from tinygpt_v2 import TinyGPT

# Hyperparameters (V2 Configuration)
EMBEDDING_DIM = 128
NUM_HEADS = 8
NUM_BLOCKS = 4
CONTEXT_LENGTH = 64
...
print(f"TinyGPT V2 initialized with {param_count:,} parameters.")
...
def generate_sample(prompt="ROMEO:"):
    ...
    generated = model.generate(idx, max_new_tokens=150, temperature=0.8, top_k=5)
    out_text = "".join([id_to_char[i.item()] for i in generated[0]])
    return out_text

if __name__ == "__main__":
    print("\n=== Training TinyGPT V2 (20 Epochs) ===")
    for epoch in range(EPOCHS):
        train_loss = train_epoch()
        val_loss = evaluate()
        print(f"Epoch {epoch + 1:02d}/{EPOCHS} | Train Loss: {train_loss:.4f} | Val Loss: {val_loss:.4f}")
        
    save_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "tinygpt_v2.pt")
